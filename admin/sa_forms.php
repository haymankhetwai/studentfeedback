    ...
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-xl font-bold text-slate-800">
                <?= $LANG['sa_forms_title'] ?? 'Student Affairs Feedback Forms' ?>
            </h2>
        </div>
        <p class="text-sm text-slate-500">
            <?= $LANG['sa_forms_subtitle'] ?? 'Manage feedback forms for student affairs office evaluation' ?>
        </p>
    </div>
    <button onclick="openModal('addModal')"
        class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <form method="GET" class="flex items-center gap-2 flex-1">
            <div class="relative flex-1 max-w-xs">
                ...
                <input type="text" name="search" value="<?= e($search) ?>"
                    placeholder="<?= $LANG['search_forms'] ?? 'Search forms...' ?>"
                    class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
            </div>
            <button type="submit"
                class="px-3 py-2 text-sm bg-purple-600 text-white rounded-xl hover:bg-purple-700"><?= $LANG['search'] ?? 'Search' ?></button>
            <?php if ($search): ?><a href="sa_forms.php"
            ...
            <thead class="bg-slate-200 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">
                        <?= $LANG['form_title_label'] ?? 'Form Title' ?>
                    </th>
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">
                        <?= $LANG['col_duration'] ?? 'Duration' ?>
                    </th>
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">
                        <?= $LANG['questions_label'] ?? 'Questions' ?>
                    </th>
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">
                ...
                <?php if ($rows):
                    foreach ($rows as $i => $row): ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-5 py-3">
                                <p class="text-sm font-medium text-slate-800"><?= e($row['title']) ?></p>
                            </td>
                            <td class="px-5 py-3 text-xs text-slate-500">
                                <?= formatDateTime($row['start_date']) ?><br><?= formatDateTime($row['end_date']) ?><br><span
                                    class="text-[10px] font-semibold"><?= badgeStatus($row['status']) ?></span>
                            </td>
                            <td class="px-5 py-3"><span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-700"><?= $row['q_count'] ?>
                                    Questions</span></td>
                            <td class="px-5 py-3"><span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700"><?= $row['s_count'] ?>
                                    <?= $LANG['submitted_label'] ?? 'submitted' ?></span></td>
                            <td class="px-5 py-3 text-xs text-slate-500"><?= getTimeRemaining($row['end_date']) ?></td>
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="sa_questions.php"
                                    ...
                                    <a href="sa_results.php?form_id=<?= $row['id'] ?>"
                                    ...
                                    <button onclick="openDelete(<?= $row['id'] ?>,<?= e(json_encode($row['title'])) ?>)"
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-16 text-slate-400">
                            ...
                            <p class="text-sm"><?= $LANG['no_sa_forms_found'] ?? 'No SA feedback forms yet.' ?></p>
                        </td>
                    </tr>
                <?php endif ?>
            </tbody>
    ...
    <div class="px-5 py-4 border-t border-slate-100">
        <?= paginationLinks($pg, 'sa_forms.php' . ($search ? '?search=' . urlencode($search) : '')) ?>
    </div>
</div>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div class="flex items-center gap-2"><?= iconSvg('shield', 'w-5 h-5 text-purple-600') ?>
                <h3 class="font-semibold text-slate-800">
                    <?= $LANG['create_sa_form_modal'] ?? 'Create SA Feedback Form' ?>
                </h3>
            </div>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div><label
                        class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['form_title_label'] ?? 'Form Title' ?>
                        <span class="text-red-500">*</span></label>
                    <input type="text" name="title" required
                        placeholder="<?= $LANG['semester_sa_evaluation'] ?? 'e.g. Semester Student Affairs Evaluation' ?>"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label
                            class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['start_date'] ?? 'Start Date & Time' ?>
                            <span class="text-red-500">*</span></label>
                        <input type="datetime-local" name="start_date" required
                            class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
                    </div>
                    <div><label
                            class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['end_date'] ?? 'End Date & Time' ?>
                            <span class="text-red-500">*</span></label>
                ...
                <div><label
                        class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['academic_year_label'] ?? 'Academic Year' ?></label>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit"
                    class="px-5 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-xl"><?= $LANG['create_sa_form'] ?? 'Create Form' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden"
            ...
            <div class="px-6 py-5 space-y-4">
                <div><label
                        class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['form_title_label'] ?? 'Form Title' ?></label>
                    <input type="text" name="title" id="edit_title" required
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div><label
                            class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['start_date'] ?? 'Start Date & Time' ?></label>
                        <input type="datetime-local" name="start_date" id="edit_start"
                            class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
                    </div>
                    <div><label
                            class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['end_date'] ?? 'End Date & Time' ?></label>
    ...
    <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
        <button type="button" onclick="closeModal('editModal')"
            class="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-100"><?= $LANG['cancel'] ?? 'Cancel' ?></button>
        <button type="submit"
            class="px-5 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-xl"><?= $LANG['save_changes'] ?? 'Save Changes' ?></button>
    </div>
    </form>
</div>
</div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm modal-box">
        <div class="px-6 py-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-800"><?= $LANG['delete_sa_form_modal'] ?? 'Delete SA Form' ?>
            </h3>
            <p class="text-sm text-slate-500 mt-2"><?= $LANG['delete'] ?? 'Delete' ?> <strong id="delete_name"
                    class="text-slate-700"></strong>?
                <?= $LANG['delete_form_submissions_lost'] ?? 'All submissions will be lost.' ?>
            </p>
        </div>
