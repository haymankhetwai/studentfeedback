...
$pageTitle  = 'Student Affairs Feedback Forms';
$activeMenu = 'sa_forms';
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-xl font-bold text-slate-800">Student Affairs Feedback Forms</h2>
        </div>
        <p class="text-sm text-slate-500">Manage feedback forms for student affairs office evaluation</p>
    </div>
    <button onclick="openModal('addModal')" class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
        <?= iconSvg('plus','w-4 h-4') ?> Create SA Form
    </button>
</div>
<?php renderFlash() ?>
    ...
    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        <form method="GET" class="flex items-center gap-2 flex-1">
            <div class="relative flex-1 max-w-xs">
                ...
                <input type="text" name="search" value="<?= e($search) ?>" placeholder="Search forms..." class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
            </div>
            <button type="submit" class="px-3 py-2 text-sm bg-purple-600 text-white rounded-xl hover:bg-purple-700">Search</button>
            <?php if ($search): ?><a href="sa_forms.php" class="px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-600">Clear</a><?php endif ?>
        </form>
        ...
        <table>
            ...
            <?php if ($rows): foreach ($rows as $i => $row): ?>
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-5 py-3"><span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-700"><?= $row['q_count'] ?> Q</span></td>
            ...
            <?php endforeach; else: ?>
                <tr><td colspan="7" class="text-center py-16 text-slate-400"><?= iconSvg('shield','w-10 h-10 mx-auto mb-3 opacity-40') ?><p class="text-sm">No SA feedback forms yet.</p></td></tr>
            <?php endif ?>
            </tbody>
        </table>
    </div>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div class="flex items-center gap-2"><?= iconSvg('shield','w-5 h-5 text-purple-600') ?><h3 class="font-semibold text-slate-800">Create SA Feedback Form</h3></div>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Form Title <span class="text-red-500">*</span></label>
                    <input type="text" name="title" required placeholder="e.g. Semester Student Affairs Evaluation" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none"></div>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-xl">Create Form</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800">Edit SA Feedback Form</h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
            <div class="px-6 py-5 space-y-4">
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Form Title</label>
