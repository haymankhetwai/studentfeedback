...
if ($semId) {
    $allTrendData = getModuleRatingTrend($conn, 'learning_environment', $semId);
    ...
    <!-- <div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-orange-50 rounded-xl p-4 text-center">
                <p class="text-2xl font-bold text-orange-700"><?= $trendData[0]['overall_rating'] ?></p>
                <p class="text-xs text-orange-600 mt-1"><?= $LANG['overall_rating'] ?? 'Overall Rating' ?> (%)</p>
            </div>
                    ...
                    <?php foreach ($trendData as $i => $row): ?>
                        <tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="py-3 px-4 text-center">
                                <span
                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-orange-50 text-orange-700 font-bold text-sm"><?= $row['overall_rating'] ?>%</span>
                            </td>
