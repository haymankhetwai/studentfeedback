...
if ($formId) {
    ...
    if ($form) {
        ...
        $q = $conn->prepare("SELECT id, question_no, question_text, question_type FROM adm_feedback_questions WHERE form_id=? ORDER BY question_no ASC");
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 mb-6 myanmar-font">
    <div class="flex-1 max-w-2xl">
        <label class="block text-xs font-bold text-slate-500 mb-1">Select Administration Form (စစ်တမ်းပုံစံ
            ရွေးချယ်ရန်):</label>
        <select onchange="location.href='?form_id='+this.value"
            ...
            <option value="">— Choose an Administration Form —</option>
    ...
    <div class="w-full max-w-5xl mx-auto myanmar-font">
        <div class="bg-white shadow-lg rounded-xl border border-slate-200 p-6 md:p-8">
            ...
            <div class="text-center border-b-2 border-slate-800 pb-4 mb-5">
                <h2 class="text-lg md:text-xl font-bold text-slate-900 mb-1">စီမံခန့်ခွဲမှုဆိုင်ရာသုံးသပ်ချက် ရလဒ်များ</h2>
                <p class="text-xs text-slate-400 font-mono">University Campus — Statistical Evaluation Matrix (Anonymous)
                </p>
            </div>
