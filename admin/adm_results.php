...
$pageTitle = 'Administration Feedback Results Matrix';
...
<style>
    ...
    @media print {
        ...
        .bg-white.shadow-lg { box-shadow: none !important; border: 1px solid #e2e8f0 !important; }
        ...
        div[class*="max-h-"] { max-height: none !important; overflow: visible !important; }
        div[class*="overflow-y-auto"] { overflow: visible !important; }
        div[class*="overflow-y-auto"][class*="max-h"] { max-height: none !important; overflow: visible !important; }
        .space-y-2 > div:last-child { page-break-inside: avoid; }
    }
</style>

<div class="mb-6 myanmar-font flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-xl font-bold text-slate-800">Administration Feedback Matrix</h2>
        <p class="text-sm text-slate-500 mt-0.5">စီမံခန့်ခွဲမှုဆိုင်ရာ စစ်တမ်းများ၏ မေးခွန်းအလိုက်
            စုစုပေါင်းစာရင်းဇယားရလဒ်များ</p>
    </div>
    <?php if ($form): ?>
...
<?php if ($semesterFilter): ?>
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 myanmar-font no-print">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 text-center">
            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">စုစုပေါင်း ကျောင်းသား (Total Students)
            </p>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 text-center">
            ...
            <p class="text-[10px] text-slate-400 mt-1">
                <?= $totalStudents > 0 ? round(($completedFeedback / $totalStudents) * 100) : 0 ?>% response rate</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 text-center">
            ...
            <p class="text-[10px] text-slate-400 mt-1">
                <?= $totalStudents > 0 ? round(($pendingFeedback / $totalStudents) * 100) : 0 ?>% remaining</p>
        </div>
    </div>
<?php endif ?>
    ...
    <div class="w-full max-w-5xl mx-auto myanmar-font">
        <div class="bg-white shadow-lg rounded-xl border border-slate-200 p-6 md:p-8">
            ...
            <div class="text-center border-b-2 border-slate-800 pb-4 mb-5">
                ...
                <p class="text-xs text-slate-400 font-mono">University Campus — Statistical Evaluation Matrix (Anonymous)</p>
            </div>

            <div
                class="bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                <div class="text-right md:text-left">
                    ...
                    <div class="font-medium text-slate-700 mt-0.5 font-mono"><?= formatDate($form['start_date']) ?> -
            ...
            <?php if (!empty($ratingQuestions)): ?>
                <div class="mb-8">
                    <h3 class="text-xs font-bold text-slate-900 uppercase tracking-wider mb-3">၁။ လုပ်ဆောင်ချက်များအလိုက်
                        စုစုပေါင်းအကဲဖြတ်မှု စာရင်းဇယား</h3>
            ...
            <?php if (!empty($commentQuestions)): ?>
                <div class="space-y-6 pt-6 border-t-2 border-slate-300">
                    <h3 class="text-xs font-bold text-slate-900 uppercase tracking-wider">၂။ အကြံပြုချက်များနှင့်
                        မှတ်ချက်များစုစည်းမှု (Comments Box)</h3>
                        ...
                        <div class="space-y-2 text-xs">
                            <label class="block font-bold text-slate-700 text-sm">
                                ...
                                <span class="text-slate-400 font-normal text-xs">(စုစုပေါင်းမှတ်ချက် -
                                    <?= count($commentsForQuestion) ?> ခု)</span>
                            </label>
                            <div
                                ...
                                <?php if (!empty($commentsForQuestion)): ?>
                                    ...
                                    <?php endforeach ?>
                                <?php else: ?>
                                    <div class="text-slate-400 italic text-center py-4">— (ဤမေးခွန်းအတွက် ကျောင်းသားများထံမှ
                                        မှတ်ချက်မရှိပါ) —</div>
                                <?php endif ?>
                            </div>
                        </div>
            ...
            <div class="mt-8 pt-4 border-t border-slate-100 text-right text-[11px] text-slate-400 font-semibold italic">
                "ဤအစီရင်ခံစာသည် စနစ်အတွင်းရှိ ကျောင်းသားများ၏ ပေးပို့ချက်အားလုံးကို စုစည်းတွက်ချက်ထားသော
                စာရင်းအင်းမူရင်းမှတ်တမ်း ဖြစ်ပါသည်။"
            </div>
        </div>
    </div>
<?php endif ?>
