...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">One question set per Academic Year and Module — shared across all semesters.</p>
    </div>
    <div class="flex gap-2">
        <!-- <button onclick="openModal('cloneModal')" class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
        ...
        <button onclick="openModal('addModal')" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm shadow-indigo-600/20 transition-all hover:-translate-y-0.5">
            <?= iconSvg('plus', 'w-4 h-4') ?> Create Question Set
        </button>
    </div>
</div>
<?php renderFlash() ?>
    ...
    <form method="GET" class="flex flex-wrap items-end gap-3">
        ...
        <div class="flex-1 min-w-[160px]">
            ...
            <select name="module" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
                ...
                <option value="student_affairs" <?= $filterMod === 'student_affairs' ? 'selected' : '' ?>>Student Affairs</option>
                <option value="administration" <?= $filterMod === 'administration' ? 'selected' : '' ?>>Administration</option>
            </select>
        </div>
        <div class="flex gap-2">
            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700">Filter</button>
            <?php if ($filterAy || $filterMod): ?>
                <a href="question_sets.php" class="px-4 py-2 bg-slate-100 text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-200">Reset</a>
            <?php endif ?>
        </div>
    </form>
</div>
        ...
        <table>
            <thead class="bg-slate-200 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">Title</th>
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">Academic Year</th>
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">Module</th>
                    <th class="text-center px-5 py-3 text-slate-500 text-sm font-semibold">Questions</th>
                    ...
                    <td class="px-5 py-3 text-center">
                        <div class="flex items-center justify-center gap-1.5">
                            <a href="<?= moduleQuestionsPage($row['module']) ?>?set_id=<?= $row['id'] ?>" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-purple-700 bg-purple-50 hover:bg-purple-100 rounded-lg">
                                <?= iconSvg('question', 'w-3.5 h-3.5') ?> View
                            </a>
            ...
            <?php endforeach; else: ?>
                <tr><td colspan="8" class="text-center py-16 text-slate-400">
                    ...
                    <p class="text-sm">No question sets found. Create one or clone from a previous year.</p>
                </td></tr>
            <?php endif ?>
            </tbody>
        </table>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800">Create Question Set</h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Academic Year <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Module <span class="text-red-500">*</span></label>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl">Create</button>
            </div>
        </form>
    </div>
</div>
...
<div id="cloneModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800">Clone Question Set from Previous Year</h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="clone">
            <div class="px-6 py-5 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Source Question Set <span class="text-red-500">*</span></label>
                    <select name="source_id" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none bg-white">
                        <option value="">Select source question set to clone from</option>
                ...
                <div class="border-t border-slate-200 pt-4">
                    <p class="text-sm font-semibold text-slate-700 mb-3">Target (New Academic Year)</p>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Title <span class="text-red-500">*</span></label>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                <button type="button" onclick="closeModal('cloneModal')" class="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-100">Cancel</button>
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-xl">Clone Question Set</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800">Edit Question Set</h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
            <div class="px-6 py-5 space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Module cannot be changed after creation.</p>
                </div>
                <div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm modal-box">
        <div class="px-6 py-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-800">Delete Question Set</h3>
            <p class="text-sm text-slate-500 mt-2">Delete <strong id="delete_name" class="text-slate-700"></strong>?</p>
            <p class="text-xs text-red-500 mt-2">Cannot delete if used by feedback forms. All questions in this set will also be deleted.</p>
        </div>
