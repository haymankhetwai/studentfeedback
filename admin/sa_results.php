...
if ($semesterFilter) {
    ...
    if ($formId && $totalStudents > 0) {
        $compStmt = $conn->prepare("SELECT COUNT(DISTINCT sfs.student_id) AS cnt FROM feedback_submissions sfs JOIN students st ON sfs.student_id = st.id JOIN section_assignments sa ON sa.student_id = st.id JOIN sections s ON sa.section_id = s.id WHERE sfs.form_id = ? AND s.semester = ?");
...
<div class="mb-6 myanmar-font flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-xl font-bold text-slate-800"><?= $LANG['sa_results_content_heading'] ?? 'Student Affairs Feedback Matrix' ?></h2>
        <p class="text-sm text-slate-500 mt-0.5"><?= $LANG['sa_results_subtitle'] ?? 'ကျောင်းသားရေးရာဌာန စစ်တမ်းများ၏ မေးခွန်းအလိုက် စုစုပေါင်းစာရင်းဇယားရလဒ်များ' ?></p>
    </div>
    <?php if ($form): ?>
