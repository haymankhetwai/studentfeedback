...
$pageTitle = $LANG['sa_results_title'] ?? 'Student Affairs Feedback Results Matrix';
...
$allForms = $conn->query("SELECT id, title, status FROM feedback_forms WHERE module='student_affairs' ORDER BY id DESC")->fetch_all(MYSQLI_ASSOC);
...
if ($formId) {
    ...
    $r = $conn->prepare("SELECT * FROM feedback_forms WHERE id = ? AND module='student_affairs'");
    ...
    if ($form) {
        ...
        $q = $conn->prepare("SELECT id, question_no, question_text, question_type, options_json FROM feedback_questions WHERE module='student_affairs' ORDER BY question_no ASC");
        $q->execute();
...
if ($semesterFilter) {
    // Total Students = all students in the selected Semester
    // Count students from all sections in the semester via section_assignments
    $stStmt = $conn->prepare("SELECT COUNT(DISTINCT sa.student_id) AS cnt FROM section_assignments sa JOIN sections s ON sa.section_id = s.id WHERE s.semester = ?");
    ...
    if ($formId) {
        $compStmt = $conn->prepare("SELECT COUNT(DISTINCT fs.student_id) AS cnt FROM feedback_submissions fs JOIN section_assignments sa ON fs.student_id = sa.student_id JOIN sections s ON sa.section_id = s.id WHERE fs.form_id = ? AND s.semester = ?");
...
<div class="mb-6 myanmar-font flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-xl font-bold text-slate-800">
            <?= $LANG['sa_results_content_heading'] ?? 'Student Affairs Feedback Matrix' ?>
        </h2>
        <p class="text-sm text-slate-500 mt-0.5">
            <?= $LANG['sa_results_subtitle'] ?? 'ကျောင်းသားရေးရာဌာန စစ်တမ်းများ၏ မေးခွန်းအလိုက် စုစုပေါင်းစာရင်းဇယားရလဒ်များ' ?>
        </p>
    </div>
    <?php if ($form): ?>
...
<?php if ($form): ?>
                        ...
                        <div class="space-y-2">
                            ...
                            <div
                                ...
                                <?php if (!empty($commentsForQuestion)): ?>
                                    ...
                                    <?php endforeach ?>
                                <?php else: ?>
                                    <div class="text-slate-400 italic text-center text-sm py-4">—
    ...
    <div class="print-only myanmar-font" style="max-width: 100%;">
        <div class="print-report-header">
            ...
            <h2>Student Affairs Feedback Evaluation Report</h2>
        ...
        <div class="print-section">
            ...
            <dl class="print-info-grid">
                ...
                <dt>Semester:</dt>
                <dd><?= e($semesterFilter ? formatSemester($semesterFilter) : ($LANG['all_semesters'] ?? 'All Semesters')) ?>
                </dd>
        ...
        <?php if (!empty($ratingQuestions) && $completedCount > 0): ?>
            <div class="print-section">
                ...
                <div class="print-rating-summary">
                    ...
                    <div class="print-rating-item">
                        <div class="value"><span class="print-grade-badge">
                                <?= $grade ?>
                            </span></div>
                ...
                <div style="font-size:8pt; color:#94a3b8; text-align:center; margin-top:4px;">
                    <?= $LANG['good'] ?? 'Good' ?> = 5 pts ·
                    <?= $LANG['fair'] ?? 'Fair' ?> = 3 pts ·
        ...
        <div class="print-section">
            ...
            <div class="print-conclusion">
                ...
                <?php
                $conclusions = [
                    'Excellent' => 'Student Affairs services have demonstrated outstanding performance based on student feedback. Students are highly satisfied with the support and services provided. It is recommended to recognize and commend this performance.',
                    'Very Good' => 'Student Affairs services have shown very good performance with high student satisfaction. Minor areas for improvement may exist, but overall service quality is well above expectations.',
                    'Good' => 'Student Affairs services have achieved a good performance rating. There are opportunities for further improvement in certain areas, but the overall feedback is positive.',
                    'Fair' => 'Student Affairs services performance is at an acceptable level. There are notable areas requiring improvement. It is recommended to provide constructive feedback and development opportunities.',
                    'Needs Improvement' => 'Student Affairs services performance falls below the expected standard. Significant improvement is needed in service delivery, student support, and overall effectiveness. Immediate attention and support are recommended.',
                ];
