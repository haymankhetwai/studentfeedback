...
$pageTitle  = $LANG['sa_questions_title'] ?? 'Student Affairs Feedback Questions';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf()) {
        ...
        if ($qno && $qtext) {
            ...
            if ($qtype === 'survey') {
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if ($opt === '') { setFlash('error','All 4 survey options are required.'); header('Location: sa_questions.php'); exit; }
                    $opts[] = $opt;
                }
                $optionsJson = json_encode($opts, JSON_UNESCAPED_UNICODE);
            }
            $chk = $conn->prepare("SELECT 1 FROM feedback_questions WHERE module=? AND question_no=? AND question_type=?");
            $chk->bind_param('sis', $module, $qno, $qtype);
            $chk->execute();
            $exists = (bool) $chk->get_result()->num_rows;
            $chk->close();
            if ($exists) {
                setFlash('error', ucfirst($qtype) . ' Question #' . $qno . ' already exists for Student Affairs. Use a different number.');
            } else {
                $stmt = $conn->prepare("INSERT INTO feedback_questions (module, question_no, question_text, question_type, options_json) VALUES (?,?,?,?,?)");
                $stmt->bind_param('sisss', $module, $qno, $qtext, $qtype, $optionsJson);
    ...
    if ($action === 'edit') {
        ...
        if ($id && $qno && $qtext) {
            ...
            $chk = $conn->prepare("SELECT 1 FROM feedback_questions WHERE module=? AND question_no=? AND question_type=? AND id!=?");
            $chk->bind_param('sisi', $module, $qno, $qtype, $id);
            $chk->execute();
            $exists = (bool) $chk->get_result()->num_rows;
            $chk->close();
            if ($exists) {
    ...
    header('Location: sa_questions.php'); exit;
}
...
$dataStmt = $conn->prepare("SELECT fq.* FROM feedback_questions fq $where ORDER BY FIELD(fq.question_type,'rating','comment','survey'), fq.question_no LIMIT ? OFFSET ?");
$dataStmt->bind_param($t2, ...$p2);
$dataStmt->execute(); $rows = $dataStmt->get_result()->fetch_all(MYSQLI_ASSOC); $dataStmt->close();

$maxRating = (int)$conn->query("SELECT COALESCE(MAX(question_no),0) AS m FROM feedback_questions WHERE module='$module' AND question_type='rating'")->fetch_assoc()['m'];
$maxComment = (int)$conn->query("SELECT COALESCE(MAX(question_no),0) AS m FROM feedback_questions WHERE module='$module' AND question_type='comment'")->fetch_assoc()['m'];
$maxSurvey = (int)$conn->query("SELECT COALESCE(MAX(question_no),0) AS m FROM feedback_questions WHERE module='$module' AND question_type='survey'")->fetch_assoc()['m'];
$nextNo = $maxRating + 1;
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-xl font-bold text-slate-800"><?= $LANG['sa_questions_title'] ?? 'SA Feedback Questions' ?></h2>
        </div>
        <p class="text-sm text-slate-500 mt-0.5"><?= $LANG['sa_questions_subtitle'] ?? 'Shared questions used by all Student Affairs feedback forms across every semester' ?></p>
    </div>
    <button onclick="openModal('addModal')" class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
        <?= iconSvg('plus','w-4 h-4') ?> <?= $LANG['add_sa_question'] ?? 'Add Question' ?>
    </button>
</div>
<?php renderFlash() ?>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        <form method="GET" class="flex items-center gap-2 flex-1">
            <div class="relative flex-1 max-w-xs"><span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400"><?= iconSvg('search','w-4 h-4') ?></span>
                <input type="text" name="search" value="<?= e($search) ?>" placeholder="<?= $LANG['search_questions'] ?? 'Search questions...' ?>" class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
            </div>
            ...
            <?php if ($search): ?><a href="sa_questions.php" class="px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-600"><?= $LANG['clear'] ?? 'Clear' ?></a><?php endif ?>
        </form>
    ...
    <div class="overflow-x-auto">
        <table>
            ...
            <?php if ($rows): foreach ($rows as $row): ?>
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-5 py-3 text-center">
                        <span class="w-7 h-7 rounded-full bg-purple-100 text-purple-700 text-xs font-bold flex items-center justify-center"><?= $row['question_no'] ?></span>
                    </td>
                    ...
                    <td class="px-5 py-3">
                        ...
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                            <?= iconSvg('star','w-3.5 h-3.5') ?> <?= $LANG['rating'] ?? 'Rating' ?>
                        </span>
                        ...
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-cyan-100 text-cyan-800">
                            <?= iconSvg('clipboard','w-3.5 h-3.5') ?> <?= $LANG['comment'] ?? 'Comment' ?>
                        </span>
                        <?php endif ?>
                    </td>
                    <td class="px-5 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button onclick="openEdit(<?= htmlspecialchars(json_encode($row), ENT_QUOTES) ?>)"
                                class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-purple-700 bg-purple-50 hover:bg-purple-100 rounded-lg">
                                <?= iconSvg('edit','w-3.5 h-3.5') ?> <?= $LANG['edit'] ?? 'Edit' ?>
                            </button>
                            <button onclick="openDelete(<?= $row['id'] ?>,'Questions <?= $row['question_no'] ?>')"
            ...
            <?php endforeach; else: ?>
                <tr><td colspan="4" class="text-center py-16 text-slate-400"><?= iconSvg('question','w-10 h-10 mx-auto mb-3 opacity-40') ?><p class="text-sm"><?= $LANG['no_questions_found'] ?? 'No questions yet. Add your first question.' ?></p></td></tr>
            <?php endif ?>
            </tbody>
        </table>
    </div>
    <div class="px-5 py-4 border-t border-slate-100"><?= paginationLinks($pg, 'sa_questions.php' . ($search ? '?search=' . urlencode($search) : '')) ?></div>
</div>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800"><?= $LANG['add_question_modal'] ?? 'Add SA Question' ?></h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_no'] ?? 'Question #' ?> <span class="text-red-500">*</span></label>
                        <input type="number" name="question_no" id="add_qno" value="<?= $nextNo ?>" required min="1" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
                    </div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_type'] ?? 'Type' ?> <span class="text-red-500">*</span></label>
                        <select name="question_type" id="add_qtype" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none bg-white">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_text'] ?? 'Question Text' ?> <span class="text-red-500">*</span></label>
                    <textarea name="question_text" required rows="3" placeholder="<?= $LANG['question_placeholder'] ?? 'Enter the question...' ?>" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none resize-none"></textarea>
                </div>
                <div id="add_survey_options" class="hidden space-y-3">
                    ...
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                    <input type="text" name="survey_option_<?= $i ?>" maxlength="255" placeholder="Option <?= $i ?>" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
                    <?php endfor; ?>
                </div>
            </div>
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-xl"><?= $LANG['add_sa_question'] ?? 'Add Question' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800"><?= $LANG['edit_sa_question_modal'] ?? 'Edit SA Question' ?></h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_no'] ?? 'Question #' ?></label>
                        <input type="number" name="question_no" id="edit_qno" min="1" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
                    </div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_type'] ?? 'Type' ?></label>
                        <select name="question_type" id="edit_qtype" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none bg-white">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_text'] ?? 'Question Text' ?></label>
                    <textarea name="question_text" id="edit_qtext" rows="3" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none resize-none"></textarea>
                </div>
                <div id="edit_survey_options" class="hidden space-y-3">
                    <label class="block text-sm font-medium text-slate-700"><?= $LANG['survey_options'] ?? 'Survey Options (4 choices)' ?> <span class="text-red-500">*</span></label>
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                    <input type="text" name="survey_option_<?= $i ?>" id="edit_survey_opt_<?= $i ?>" maxlength="255" placeholder="Option <?= $i ?>" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none">
                    <?php endfor; ?>
                </div>
            </div>
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-xl"><?= $LANG['save'] ?? 'Save' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm modal-box">
        <div class="px-6 py-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-800"><?= $LANG['delete_sa_question_modal'] ?? 'Delete SA Question' ?></h3>
