...
if ($search) {
    $s2 = "%$search%";
    $stmt = $conn->prepare("SELECT f.*, (SELECT COUNT(*) FROM feedback_questions WHERE module='administration') AS q_count, (SELECT COUNT(*) FROM feedback_submissions WHERE form_id=f.id) AS s_count FROM feedback_forms f WHERE f.module='administration' AND f.title LIKE ? ORDER BY f.id DESC LIMIT ? OFFSET ?");
    $stmt->bind_param('sii', $s2, $perPage, $off);
} else {
    $stmt = $conn->prepare("SELECT f.*, (SELECT COUNT(*) FROM feedback_questions WHERE module='administration') AS q_count, (SELECT COUNT(*) FROM feedback_submissions WHERE form_id=f.id) AS s_count FROM feedback_forms f WHERE f.module='administration' ORDER BY f.id DESC LIMIT ? OFFSET ?");
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1"><?= iconSvg('office','w-5 h-5 text-orange-600') ?>
            <h2 class="text-xl font-bold text-slate-800"><?= $LANG['adm_forms_title'] ?? 'Administration Feedback Forms' ?></h2></div>
        <p class="text-sm text-slate-500"><?= $LANG['adm_forms_subtitle'] ?? 'Manage feedback forms for administration office evaluation' ?></p>
    </div>
    <button onclick="openModal('addModal')" class="inline-flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
        <?= iconSvg('plus','w-4 h-4') ?> <?= $LANG['create_adm_form'] ?? 'Create Adm Form' ?>
    </button>
</div>
<?php renderFlash() ?>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        <form method="GET" class="flex items-center gap-2 flex-1">
            <div class="relative flex-1 max-w-xs"><span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400"><?= iconSvg('search','w-4 h-4') ?></span>
                <input type="text" name="search" value="<?= e($search) ?>" placeholder="<?= $LANG['search_forms'] ?? 'Search forms...' ?>" class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none"></div>
    ...
    <div class="overflow-x-auto">
        <table>
            ...
            <?php endforeach; else: ?>
                <tr><td colspan="7" class="text-center py-16 text-slate-400"><?= iconSvg('office','w-10 h-10 mx-auto mb-3 opacity-40') ?><p class="text-sm"><?= $LANG['no_adm_forms_found'] ?? 'No administration feedback forms yet.' ?></p></td></tr>
            <?php endif ?>
            </tbody>
        </table>
    </div>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div class="flex items-center gap-2"><?= iconSvg('office','w-5 h-5 text-orange-600') ?><h3 class="font-semibold text-slate-800"><?= $LANG['create_adm_form_modal'] ?? 'Create Administration Form' ?></h3></div>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-xl"><?= $LANG['create_adm_form'] ?? 'Create Form' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800"><?= $LANG['edit_adm_form_modal'] ?? 'Edit Administration Form' ?></h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                <button type="button" onclick="closeModal('editModal')" class="px-4 py-2 text-sm text-slate600 border border-slate-200 rounded-xl hover:bg-slate-100"><?= $LANG['cancel'] ?? 'Cancel' ?></button>
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-xl"><?= $LANG['save_changes'] ?? 'Save Changes' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm modal-box">
        <div class="px-6 py-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-800"><?= $LANG['delete_adm_form_modal'] ?? 'Delete Adm Form' ?></h3>
