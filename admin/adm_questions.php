...
$allForms = $conn->query("SELECT * FROM adm_feedback_forms ORDER BY id DESC")->fetch_all(MYSQLI_ASSOC);

// ── No form selected: show form picker ────────────────────────
if (!$formId) {
    $pageTitle  = 'Adm Questions — Select Form';
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 mb-1"><?= iconSvg('office','w-5 h-5 text-orange-600') ?>
            <h2 class="text-xl font-bold text-slate-800">Administration Questions</h2></div>
        <p class="text-sm text-slate-500">Select an Administration form to manage its questions</p>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    <?php if ($allForms): foreach ($allForms as $f):
        $qCount = (int)$conn->query("SELECT COUNT(*) AS c FROM adm_feedback_questions WHERE form_id={$f['id']}")->fetch_assoc()['c'];
    ?>
        <a href="adm_questions.php?form_id=<?= $f['id'] ?>" class="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all p-5 flex flex-col gap-3">
            <div class="flex items-start justify-between gap-2">
                <div class="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center flex-shrink-0"><?= iconSvg('office','w-5 h-5 text-orange-600') ?></div>
            ...
            <div class="flex items-center justify-between">
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700"><?= $qCount ?> question<?= $qCount !== 1 ? 's' : '' ?></span>
                <span class="text-xs font-semibold text-orange-600">Manage →</span>
            </div>
        </a>
    <?php endforeach; else: ?>
        <div class="col-span-3 bg-white rounded-2xl border border-slate-100 shadow-sm text-center py-16 text-slate-400">
            <?= iconSvg('office','w-10 h-10 mx-auto mb-3 opacity-30') ?>
            <p class="text-sm">No Administration forms yet. <a href="adm_forms.php" class="text-orange-600 hover:underline">Create one first →</a></p>
        </div>
    <?php endif ?>
    </div>
    <?php
...
// ── Form selected — proceed normally ──────────────────────────
$formRow = $conn->query("SELECT * FROM adm_feedback_forms WHERE id=$formId")->fetch_assoc();
if (!$formRow) { setFlash('error','Administration form not found.'); header('Location: adm_questions.php'); exit; }

$pageTitle  = 'Adm Questions — ' . $formRow['title'];
$activeMenu = 'adm_questions';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf()) {
    ...
    if ($action === 'add') {
        ...
        if ($text) {
            $stmt = $conn->prepare("INSERT INTO adm_feedback_questions (form_id,question_no,question_text,question_type) VALUES (?,?,?,?)");
    ...
    if ($action === 'edit') {
        ...
        $type = in_array($_POST['question_type'], ['rating','comment']) ? $_POST['question_type'] : 'rating';
        if ($id && $text) {
            $stmt = $conn->prepare("UPDATE adm_feedback_questions SET question_no=?,question_text=?,question_type=? WHERE id=? AND form_id=?");
    ...
    if ($action === 'delete') {
        $id = (int)($_POST['id'] ?? 0);
        if ($id) {
            $stmt = $conn->prepare("DELETE FROM adm_feedback_questions WHERE id=? AND form_id=?");
            $stmt->bind_param('ii', $id, $formId);
            $stmt->execute() ? setFlash('success','Question deleted.') : setFlash('error','Failed.');
            $stmt->close();
        }
    }
    header("Location: adm_questions.php?form_id=$formId"); exit;
}

$questions = $conn->query("SELECT * FROM adm_feedback_questions WHERE form_id=$formId ORDER BY question_no")->fetch_all(MYSQLI_ASSOC);
...
<div class="mb-6">
    <a href="adm_questions.php" class="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-orange-600 mb-3 transition-colors">← All Adm Forms</a>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 mb-1"><?= iconSvg('office','w-5 h-5 text-orange-600') ?><h2 class="text-xl font-bold text-slate-800">Administration Questions</h2></div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="overflow-x-auto">
        <table>
            ...
            <?php endforeach; else: ?>
                <tr><td colspan="4" class="text-center py-14 text-slate-400"><?= iconSvg('question','w-10 h-10 mx-auto mb-3 opacity-40') ?><p class="text-sm">No questions yet.</p></td></tr>
            <?php endif ?>
            </tbody>
        </table>
    </div>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-3 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Q No.</label><input type="number" name="question_no" value="<?= $nextNo ?>" min="1" class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none"></div>
                    <div class="col-span-2"><label class="block text-sm font-medium text-slate-700 mb-1">Type</label>
                        <select name="question_type" class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none bg-white">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Question Text <span class="text-red-500">*</span></label>
                    <textarea name="question_text" required rows="3" placeholder="Enter your question..." class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none resize-none"></textarea></div>
            </div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-3 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Q No.</label><input type="number" name="question_no" id="edit_no" min="1" class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none"></div>
                    <div class="col-span-2"><label class="block text-sm font-medium text-slate-700 mb-1">Type</label>
                        <select name="question_type" id="edit_type" class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none bg-white">
                            <option value="rating">★ Rating</option><option value="comment">✎ Comment</option>
                        </select></div>
                </div>
                <div><label class="block text-sm font-medium text-slate-700 mb-1">Question Text</label>
                    <textarea name="question_text" id="edit_text" required rows="3" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-orange-500 outline-none resize-none"></textarea></div>
            </div>
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
