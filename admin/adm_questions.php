...
$pageTitle  = 'Global Administration Feedback Questions';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf()) {
    ...
    if ($action === 'add') {
        ...
        if ($qno && $qtext) {
            $stmt = $conn->prepare("INSERT INTO global_adm_feedback_questions (question_no, question_text, question_type) VALUES (?,?,?)");
    ...
    if ($action === 'edit') {
        ...
        $qtype = in_array($_POST['question_type'], ['rating','comment']) ? $_POST['question_type'] : 'rating';
        if ($id && $qno && $qtext) {
            $stmt = $conn->prepare("UPDATE global_adm_feedback_questions SET question_no=?,question_text=?,question_type=? WHERE id=?");
    ...
    if ($action === 'delete') {
        $id = (int)($_POST['id'] ?? 0);
        if ($id) {
            $stmt = $conn->prepare("DELETE FROM global_adm_feedback_questions WHERE id=?");
...
$params = [];
$types  = '';
if ($search) { $s2 = "%$search%"; $conds[] = "aq.question_text LIKE ?"; $params[] = $s2; $types .= 's'; }
$where = $conds ? 'WHERE ' . implode(' AND ', $conds) : '';

$cntStmt = $conn->prepare("SELECT COUNT(*) AS c FROM global_adm_feedback_questions aq $where");
...
$dataStmt = $conn->prepare("SELECT aq.* FROM global_adm_feedback_questions aq $where ORDER BY aq.question_no LIMIT ? OFFSET ?");
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-xl font-bold text-slate-800">Administration Global Questions</h2>
        </div>
