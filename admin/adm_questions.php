...
$pageTitle  = $LANG['adm_questions_title'] ?? 'Administration Feedback Questions';
        ...
        if ($qno && $qtext) {
            ...
            if ($qtype === 'survey') {
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if ($opt === '') { setFlash('error','All 4 survey options are required.'); header('Location: adm_questions.php'); exit; }
                    $opts[] = $opt;
                }
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-xl font-bold text-slate-800"><?= $LANG['adm_questions_title'] ?? 'Administration Feedback Questions' ?></h2>
        </div>
        <p class="text-sm text-slate-500 mt-0.5"><?= $LANG['adm_questions_subtitle'] ?? 'Shared questions used by all Administration feedback forms across every semester' ?></p>
    </div>
    <button onclick="openModal('addModal')" class="inline-flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
        <?= iconSvg('plus','w-4 h-4') ?> <?= $LANG['add_adm_question'] ?? 'Add Question' ?>
    </button>
</div>
<?php renderFlash() ?>
    ...
    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        <form method="GET" class="flex items-center gap-2 flex-1">
            <div class="relative flex-1 max-w-xs"><span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400"><?= iconSvg('search','w-4 h-4') ?></span>
                <input type="text" name="search" value="<?= e($search) ?>" placeholder="<?= $LANG['search_questions'] ?? 'Search questions...' ?>" class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none">
            </div>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800"><?= $LANG['add_question_modal'] ?? 'Add Administration Question' ?></h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_no'] ?? 'Question #' ?> <span class="text-red-500">*</span></label>
                        <input type="number" name="question_no" id="add_qno" value="<?= $nextNo ?>" required min="1" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none">
                    </div>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-xl"><?= $LANG['add_adm_question'] ?? 'Add Question' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800"><?= $LANG['edit_adm_question_modal'] ?? 'Edit Administration Question' ?></h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-xl"><?= $LANG['save'] ?? 'Save' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm modal-box">
        <div class="px-6 py-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-800"><?= $LANG['delete_adm_question_modal'] ?? 'Delete Administration Question' ?></h3>
