...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        <form method="GET" class="flex items-center gap-2 flex-1">
            <div class="relative flex-1 max-w-xs">
                ...
                <input type="text" name="search" value="<?= e($search) ?>" placeholder="Search academic years..." class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none">
            </div>
            <button type="submit" class="px-3 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700">Search</button>
            <?php if ($search): ?><a href="academic_years.php" class="px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-600">Clear</a><?php endif ?>
        </form>
        ...
        <table>
            ...
            <?php if ($rows): foreach ($rows as $i => $row): ?>
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-5 py-3 text-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-700"><?= $row['question_set_count'] ?></span>
                    </td>
                    <td class="px-5 py-3 text-center">
    ...
    <div class="px-5 py-4 border-t border-slate-100"><?= paginationLinks($pg, 'academic_years.php' . ($search ? '?search=' . urlencode($search) : ''), $perPage) ?></div>
</div>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md modal-box">
        ...
        <form method="POST" onsubmit="return validateYear(this)"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Year Name <span class="text-red-500">*</span></label>
                    <input type="text" name="year_name" required placeholder="e.g. 2025-2026" pattern="\d{4}-\d{4}" maxlength="9"
                    ...
                    <p class="text-xs text-slate-400 mt-1">Format: YYYY-YYYY (e.g. 2025-2026). Second year must be exactly one year after the first.</p>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl">Add Year</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md modal-box">
        ...
        <form method="POST" onsubmit="return validateYear(this)"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit_id">
            <div class="px-6 py-5 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Year Name</label>
