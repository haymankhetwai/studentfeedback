    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <div class="flex items-center gap-4 text-sm text-slate-500">
                <span class="flex items-center gap-1"><?= iconSvg('academic', 'w-4 h-4 text-indigo-500') ?>
                    <?= e($setData['year_name']) ?></span>
                <span class="flex items-center gap-1"><?= moduleBadge($module) ?></span>
                <span
                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-cyan-100 text-cyan-700"><?= $total ?><?= $LANG["questions"] ?? "Questions" ?></span>
            </div>
        </div>
        <div class="flex gap-2">
            <button onclick="openModal('previewModal')"
                class="inline-flex items-center gap-2 bg-slate-600 hover:bg-slate-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
    ...
    <form method="GET" class="flex items-center gap-3">
        ...
        <div class="relative flex-1 max-w-md">
            ...
            <input type="text" name="search" value="<?= e($search) ?>"
                placeholder="<?= $LANG['search_questions_placeholder'] ?? 'Search questions...' ?>"
        ...
        <button type="submit"
            class="px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700"><?= $LANG['filter'] ?? 'Search' ?></button>
        <?php if ($search): ?>
            <a href="manage_questions.php?set_id=<?= $filterSet ?>"
                class="px-4 py-2 bg-slate-100 text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-200"><?= $LANG['clear'] ?? 'Clear' ?></a>
        <?php endif ?>
    </form>
</div>
            ...
            <thead class="bg-slate-200 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold">
                        <?= $LANG["question_text"] ?? "Question Text" ?></th>
                    <th class="text-left px-5 py-3 text-slate-500 text-sm font-semibold"><?= $LANG["type"] ?? "Type" ?>
                    </th>
                ...
                <?php if ($rows):
                    foreach ($rows as $i => $row): ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-5 py-3 text-center">
                                <span
                                    class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold flex items-center justify-center"><?= $row['question_no'] ?></span>
                            </td>
                            ...
                            <td class="px-5 py-3">
                                ...
                                <?php elseif ($row['question_type'] === 'survey'): ?>
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-violet-100 text-violet-800">
                                        <?= iconSvg('clipboard', 'w-3.5 h-3.5') ?>            <?= $LANG["survey_mcq"] ?? "Survey (MCQ)" ?></span>
                                <?php else: ?>
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-cyan-100 text-cyan-800">
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-16 text-slate-400">
                            ...
                            <button onclick="openModal('addModal')"
                                class="mt-3 inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-xl">
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Question # <span
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Question Text <span
                            class="text-red-500">*</span></label>
                ...
                <div id="add_survey_options" class="hidden space-y-3">
                    <label class="block text-sm font-medium text-slate-700">Survey Options (4 choices) <span
                            class="text-red-500">*</span></label>
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                        <input type="text" name="survey_option_<?= $i ?>" maxlength="255" placeholder="Option <?= $i ?>"
                            class="w-full border border-slate-200 rounded-xl px-4 py-3  focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none">
                    <?php endfor ?>
                </div>
            </div>
            <div class="flex gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit"
                    class="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl"><?= $LANG['add_question'] ?? 'Add Question' ?></button>
            </div>
        </form>
    </div>
</div>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="edit"><input type="hidden"
            ...
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG["type"] ?? "Type" ?></label>
                ...
                <div>
                    <label
                        class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG["question_text"] ?? "Question Text" ?></label>
                ...
                <div id="edit_survey_options" class="hidden space-y-3">
                    <label class="block text-sm font-medium text-slate-700">Survey Options (4 choices) <span
                            class="text-red-500">*</span></label>
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                        <input type="text" name="survey_option_<?= $i ?>" id="edit_survey_opt_<?= $i ?>" maxlength="255"
...
<!-- Preview Modal -->
<div id="previewModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop"
    data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl modal-box max-h-[80vh] flex flex-col">
        ...
        <div class="px-6 py-5 overflow-y-auto flex-1" id="previewContent">
            ...
            foreach ($previewRows as $pr):
                ...
                <div class="flex items-start gap-3 py-2 border-b border-slate-50 last:border-0">
                    <span
                        class="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5"><?= $pr['question_no'] ?></span>
                        ...
                        <?php if ($pr['question_type'] === 'survey' && $pr['options_json']): ?>
                            <div class="mt-1.5 flex flex-wrap gap-1.5">
                                <?php foreach (json_decode($pr['options_json'], true) as $oi => $opt): ?>
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-violet-50 text-violet-700 border border-violet-200"><?= ($oi + 1) . '. ' . e($opt) ?></span>
                                <?php endforeach ?>
                            </div>
                        <?php endif ?>
                    </div>
        ...
        <div class="flex justify-end px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl flex-shrink-0">
            <button onclick="closeModal('previewModal')"
                class="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl"><?= $LANG['close'] ?? 'Close' ?></button>
        </div>
    </div>
</div>
