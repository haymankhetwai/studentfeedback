...
$pageTitle  = $LANG['academic_questions_title'] ?? 'Academic Feedback Questions';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf()) {
        ...
        if ($qno && $qtext) {
            ...
            if ($qtype === 'survey') {
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if ($opt === '') { setFlash('error','All 4 survey options are required.'); header('Location: feedback_questions.php'); exit; }
                    $opts[] = $opt;
                }
            ...
            $chk->close();
            if ($exists) {
                setFlash('error', ucfirst($qtype) . ' Question #' . $qno . ' already exists for Academic. Use a different number.');
            } else {
    ...
    header('Location: feedback_questions.php'); exit;
}
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-xl font-bold text-slate-800"><?= $LANG['academic_questions_title'] ?? 'Academic Feedback Questions' ?></h2>
        </div>
        <p class="text-sm text-slate-500 mt-0.5"><?= $LANG['academic_questions_subtitle'] ?? 'Shared questions used by all academic feedback forms across every teacher and semester' ?></p>
    </div>
    <button onclick="openModal('addModal')" class="inline-flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm shadow-cyan-600/20 transition-all hover:-translate-y-0.5">
        <?= iconSvg('plus','w-4 h-4') ?> <?= $LANG['add_question'] ?? 'Add Question' ?>
    </button>
</div>
<?php renderFlash() ?>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        <form method="GET" class="flex items-center gap-2 flex-1">
            <div class="relative flex-1 max-w-xs"><span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400"><?= iconSvg('search','w-4 h-4') ?></span>
                <input type="text" name="search" value="<?= e($search) ?>" placeholder="<?= $LANG['search_questions'] ?? 'Search questions...' ?>" class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none">
            </div>
            <button type="submit" class="px-3 py-2 text-sm bg-cyan-600 text-white rounded-xl hover:bg-cyan-700"><?= $LANG['search'] ?? 'Search' ?></button>
            <?php if ($search): ?><a href="feedback_questions.php" class="px-3 py-2 text-sm border border-slate-200 rounded-xl text-slate-600"><?= $LANG['clear'] ?? 'Clear' ?></a><?php endif ?>
        </form>
        <span class="text-xs text-slate-400"><?= $total ?> <?= $LANG['records'] ?? 'question' ?><?= $total!==1?'s':'' ?></span>
    </div>
            ...
            <?php if ($rows): foreach ($rows as $row): ?>
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-5 py-3 text-center">
                        <span class="w-7 h-7 rounded-full bg-cyan-100 text-cyan-700 text-xs font-bold flex items-center justify-center"><?= $row['question_no'] ?></span>
                    </td>
    ...
    <div class="px-5 py-4 border-t border-slate-100"><?= paginationLinks($pg,'feedback_questions.php'.($search?'?search='.urlencode($search):'')) ?></div>
</div>
...
<div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800"><?= $LANG['add_question_modal'] ?? 'Add Question' ?></h3>
        ...
        <form method="POST"><?= csrfField() ?><input type="hidden" name="action" value="add">
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_no'] ?? 'Question #' ?> <span class="text-red-500">*</span></label>
                        <input type="number" name="question_no" id="add_qno" value="<?= $nextNo ?>" required min="1" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none">
                    </div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_type'] ?? 'Type' ?> <span class="text-red-500">*</span></label>
                        <select name="question_type" id="add_qtype" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none bg-white">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_text'] ?? 'Question Text' ?> <span class="text-red-500">*</span></label>
                    <textarea name="question_text" required rows="3" placeholder="<?= $LANG['question_placeholder'] ?? 'Enter the question...' ?>" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none resize-none"></textarea>
                </div>
                <div id="add_survey_options" class="hidden space-y-3">
                    ...
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                    <input type="text" name="survey_option_<?= $i ?>" maxlength="255" placeholder="Option <?= $i ?>" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none">
                    <?php endfor; ?>
                </div>
            </div>
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-cyan-600 hover:bg-cyan-700 rounded-xl"><?= $LANG['add_question'] ?? 'Add Question' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg modal-box">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800"><?= $LANG['edit_question_modal'] ?? 'Edit Question' ?></h3>
            ...
            <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_no'] ?? 'Question #' ?></label>
                        <input type="number" name="question_no" id="edit_qno" min="1" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none">
                    </div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_type'] ?? 'Type' ?></label>
                        <select name="question_type" id="edit_qtype" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none bg-white">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-1"><?= $LANG['question_text'] ?? 'Question Text' ?></label>
                    <textarea name="question_text" id="edit_qtext" rows="3" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none resize-none"></textarea>
                </div>
                <div id="edit_survey_options" class="hidden space-y-3">
                    <label class="block text-sm font-medium text-slate-700">Survey Options (4 choices) <span class="text-red-500">*</span></label>
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                    <input type="text" name="survey_option_<?= $i ?>" id="edit_survey_opt_<?= $i ?>" maxlength="255" placeholder="Option <?= $i ?>" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none">
                    <?php endfor; ?>
                </div>
            </div>
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-cyan-600 hover:bg-cyan-700 rounded-xl"><?= $LANG['save'] ?? 'Save' ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4 modal-backdrop" data-modal-backdrop>
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm modal-box">
        <div class="px-6 py-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-800"><?= $LANG['delete_question_modal'] ?? 'Delete Question' ?></h3>
